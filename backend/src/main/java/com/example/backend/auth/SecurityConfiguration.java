...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http)
        ...
        http.authorizeHttpRequests(customizer -> {
            customizer
                .requestMatchers(antMatcher(HttpMethod.POST, "/api/users"))
                .permitAll()
                .requestMatchers(
                    antMatcher(HttpMethod.GET, "/api/users/verify-email")
                )
                .permitAll()
                .requestMatchers(
                    antMatcher(HttpMethod.POST, "/api/users/forgot-password")
                )
                .permitAll()
                .requestMatchers(
                    antMatcher(HttpMethod.PATCH, "/api/users/reset-password")
                )
                .permitAll()
                .requestMatchers(antMatcher(HttpMethod.POST, "/api/auth/login"))
                .permitAll()
                .requestMatchers(antMatcher(HttpMethod.GET, "/api/auth/csrf"))
                .permitAll()
                .requestMatchers(
                    antMatcher(HttpMethod.GET, "/api/auth/impersonate")
                )
                .hasRole("ADMIN")
                .requestMatchers(
                    antMatcher(HttpMethod.GET, "/api/auth/impersonate/exit")
                )
        ...
        return new CorsConfigurationSource() {
            ...
            public CorsConfiguration getCorsConfiguration(
                ...
                config.setAllowedOrigins(
                    applicationProperties.getAllowedOrigins()
                );
    ...
    public SwitchUserFilter switchUserFilter() {
        ...
        filter.setSuccessHandler((request, response, authentication) -> {
            response.sendRedirect(
                applicationProperties.getBaseUrl() + "/switch-success"
            );
        });
        return filter;
    }
